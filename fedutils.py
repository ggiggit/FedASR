...
def fed_get_val_file_name_base(dataset, rd, ck_val,N_rd_str_format,  N_ck_str_format_val):
    
    file_name = "valid_" + dataset +"_rd"+format(rd, N_rd_str_format) + "_global"
    if ck_val is None:
        file_name += "*"
    else:
        file_name += "_ck" + format(ck_val, N_ck_str_format_val)
...
def fed_get_val_lst_file_path(
    out_folder, valid_data,rd, ck_val, fea_name, N_rd_str_format,  N_ck_str_format_val
):
    def _get_val_lst_file_name(
        dataset,  rd, ck_val, fea_name,  N_rd_str_format,N_ck_str_format_val
    ):
        file_name = fed_get_val_file_name_base(
            dataset,rd,ck_val, N_rd_str_format, N_ck_str_format_val
        )
        file_name += "_"
    ...
    lst_file_name = _get_val_lst_file_name(
        valid_data,  rd, ck_val, fea_name,N_rd_str_format,N_ck_str_format_val
    )
...
def fed_get_val_info_file_path(
    out_folder, valid_data, rd, ck_val,N_rd_str_format,  N_ck_str_format_val
):
    def _get_val_info_file_name(dataset,rd, ck_val,N_rd_str_format, N_ck_str_format_val):
        file_name = fed_get_val_file_name_base(
            dataset, rd, ck_val,N_rd_str_format, N_ck_str_format_val
        )
        file_name += ".info"
        return file_name
    info_file_name = _get_val_info_file_name(
        valid_data, rd, ck_val, N_rd_str_format, N_ck_str_format_val
    )
    info_file = out_folder + "/exp_files/" + info_file_name
    return info_file
...
def fed_get_val_cfg_file_path(
    out_folder, valid_data,rd, ck_val, N_rd_str_format, N_ck_str_format_val
):
    def _get_val_cfg_file_name(dataset,rd, ck_val,N_rd_str_format, N_ck_str_format_val):
        file_name = fed_get_val_file_name_base(
            dataset,rd,  ck_val,N_rd_str_format, N_ck_str_format_val
        )
        file_name += ".cfg"
...
def create_configs(config):
    ...
    for clt in range(N_clt):
        chunk_lst[clt] = out_folder + "/exp_files/"+"clt"+format(clt, N_clt_str_format)+"_list_chunks.txt"
        lst_chunk_file[clt] = open(chunk_lst[clt], "w")
    global_chunk_lst = out_folder + "/exp_files/"+"global"+"_list_chunks.txt"
    global_lst_chunk_file = open(global_chunk_lst, "w")
    

    # Read the batch size string
    batch_size_tr_str = config["batches"]["batch_size_train"]
    batch_size_tr_arr = expand_str_rd(batch_size_tr_str, "int", N_rd, "|", "*")

    # Read the max_seq_length_train
    if len(max_seq_length_train.split(",")) == 1:
        max_seq_length_tr_arr = expand_str_rd(max_seq_length_train, "int",N_rd, "|", "*")
    else:
        max_seq_length_tr_arr = [max_seq_length_train] * N_ep*N_rd
    ...
    for clt in range(N_clt):
        for arch in arch_lst:
            lr_arr = expand_str_rd(config[arch]["arch_lr"], "float",N_rd, "|", "*")
            lr[arch] = lr_arr
            ...
            #exp/TIMIT_fed_client3_rd24_ep1/exp_files/train_TIMIT_tr_rd00_ep0_clt0_ck0_architecture1.pkl
            pt_files[clt][arch] = config[arch]["arch_pretrain_file"]
            # for tr_data in tr_data_lst:
                # pt_files[clt][arch] = out_folder + "/exp_files/train_"+tr_data+"_rd"+format(0,N_rd_str_format)+"_ep"+format(0,N_ep_str_format)+"_clt"+format(clt, N_clt_str_format)+"_ck0_"+arch+".pkl"
            ...
            for (field_key, field_val) in config.items(arch):
                if "_drop" in field_key:
                    ...
                    for lay_id in range(N_lay):
                        drop_rates[arch].append(expand_str_rd(drop_lay[lay_id], "float", N_rd,"|", "*"))
    ...
    for rd in range(N_rd):
            ...
            for ep in range(N_ep):
                for tr_data in tr_data_lst:
                    ...
                    for ck in range(N_ck_tr):
                        ...
                        #     f.write(f"ep{ep} clt{clt} ck{ck}\n")
                        #     string = model_files[clt][arch]
                        #     f.write(string)
                        #     f.write("\n")
                        ...
                        lst_chunk_file[clt].write(config_chunk_file + "\n")
        ...
        for valid_data in valid_data_lst:
            ...
            for ck_val in range(N_ck_valid):
                lst_file = fed_get_val_lst_file_path(
                    out_folder,
                    valid_data,
                    rd,
                    ck_val,
                ...
                global_lst_chunk_file.write(config_chunk_file + "\n")
                global_model_files = dict()
                for arch in pt_files[clt].keys():
                    global_model_files[arch] = info_file_tr.replace(".info", "_" + arch + ".pkl")
...
def fed_create_lists(config):
    ...
    def _create_client_data(seq, size):
        newseq = []
        splitsize = 1.0 / size * len(seq)
        for i in range(size):
            newseq.append(seq[int(round(i * splitsize)) : int(round((i + 1) * splitsize))])
    ...
    for dataset in tr_data_name:
        ...
        full_list = []
        client_full_list = dict()
        temp_list = []
        for i in range(len(fea_names)):
            full_list.append([line.rstrip("\n") + "," for line in open(list_fea[i])])
            full_list[i] = sorted(full_list[i])
            random.shuffle(full_list[i])
            temp_list.append(_create_client_data(full_list[i],N_clt))

        full_list_fea_conc = dict()
        for clt in range(N_clt):
            client_full_list.setdefault(clt, [])
            for i in range(len(fea_names)):
                client_full_list[clt].append(temp_list[i][clt])
            # concatenating all the featues in a single file (useful for shuffling consistently)
            full_list_fea_conc[clt] = client_full_list[clt][0]
            for i in range(1, len(client_full_list[clt])):
                full_list_fea_conc[clt] = list(map(str.__add__, full_list_fea_conc[clt], client_full_list[clt][i]))
        for rd in range(N_rd):
            for clt in range(N_clt):
                ...
                # random.shuffle(full_list_fea_conc)
                #  分客户端（随机）
                # tr_client_fea = list(_create_client_data(full_list_fea_conc, N_clt))
                #  客户端分块
                for ep in range(N_ep):
                    random.shuffle(full_list_fea_conc[clt])
                
                    tr_chunks_fea = list(split_chunks(full_list_fea_conc[clt], N_chunks))
                    tr_chunks_fea.reverse()
            ...
            for dataset_val in valid_data_name:
                ...
                for ck_val in range(N_chunks_val):
                    for fea_idx in range(len(fea_names)):
                        ...
                        for snt in valid_chunks_fea[ck_val]:
                            valid_chunks_fea_split.append(snt.split(",")[fea_idx])
                        output_lst_file = fed_get_val_lst_file_path(
                            out_folder,
                            dataset_val,
                            rd,
                            ck_val,
...
def write_cfg_chunk(
    ...
    config_chunk["exp"]["seed"] = str(int(config_chunk["exp"]["seed"]) +rd+ ep + ck)
    ...
    for lr_arch in lr.keys():
        config_chunk[lr_arch]["arch_lr"] = str(lr[lr_arch][rd])
        ...
        for (field_key, field_val) in config.items(lr_arch):
            if "_drop" in field_key:
                ...
                for lay in range(N_lay):
                    drop_arr.append(drop_rates[lr_arch][lay][rd])
...
def compute_n_chunks(out_folder, data_list, rd, N_rd_str_format, step):
    list_ck = sorted(
        glob.glob(out_folder + "/exp_files/" + step + "_" + data_list + "_rd" + format(rd, N_rd_str_format) + "*.lst")
    )
    last_ck = list_ck[-1]
    N_ck = int(re.findall("_ck(.+)_", last_ck)[-1].split("_")[0]) + 1
    return N_ck

def fed_compute_n_chunks(out_folder, data_list, rd,N_rd_str_format, ep, N_ep_str_format,clt, N_clt_str_format, step):
    # def print_parameters(out_folder, data_list, ep, N_ep_str_format, clt, N_clt_str_format, step):
    #     print(f"out_folder: {out_folder}")
    #     print(f"data_list: {data_list}")
    #     print(f"ep: {ep}")
    #     print(f"N_ep_str_format: {N_ep_str_format}")
    #     print(f"clt: {clt}")
    #     print(f"N_clt_str_format: {N_clt_str_format}")
    #     print(f"step: {step}")
    #     print(out_folder, data_list, ep, N_ep_str_format,clt, N_clt_str_format, step)
    # print_parameters(out_folder, data_list, ep, N_ep_str_format, clt, N_clt_str_format, step)
    list_ck = sorted(
        glob.glob(out_folder + "/exp_files/" + step + "_" + data_list + "_rd"+format(rd, N_rd_str_format)+"_ep" + format(ep, N_ep_str_format) + "_clt" +format(clt, N_clt_str_format) + "*.lst")
    )
    last_ck = list_ck[-1]
    N_ck = int(re.findall("_ck(.+)_", last_ck)[-1].split("_")[0]) + 1
    return N_ck
...
def fed_dump_epoch_results(
    res_file_path, rd, ep, clt, tr_data_lst, tr_loss_tot, tr_error_tot, tot_time, valid_data_lst, valid_peformance_dict, lr,N_rd, N_ep, N_clt
):
    ...
    res_file = open(res_file_path, "a")
    res_file.write(
        "rd=%s ep=%s clt=%s tr=%s loss=%s err=%s "
        % (
            format(rd, N_rd_str_format),
            format(ep, N_ep_str_format),
            format(clt, N_clt_str_format),
    ...
    for lr_arch in lr.keys():
        res_file.write("lr_%s=%s " % (lr_arch, lr[lr_arch][ep + rd*N_ep]))
        print("Learning rate on %s = %s " % (lr_arch, lr[lr_arch][ep + rd*N_ep]))
    print("-----")
    res_file.write("time(s)=%i\n" % (int(tot_time)))
    print("Elapsed time (s) = %i\n" % (int(tot_time)))
    print(" ")
...
def fed_dump_round_results(
    res_file_path, rd, tr_data_lst, tr_loss_tot, tr_error_tot, tot_time, valid_data_lst, valid_peformance_dict, lr,N_rd
):
    ...
    print(" ")
    print("----- Summary round %s / %s" % (format(rd, N_rd_str_format), format(N_rd - 1, N_rd_str_format)))
    print("Training on %s" % (tr_data_lst))
    print(
        "Loss = %s | err = %s "
        % (format(tr_loss_tot / len(tr_data_lst), "0.3f"), format(tr_error_tot / len(tr_data_lst), "0.3f"))
    ...
    for lr_arch in lr.keys():
        res_file.write("lr_%s=%s " % (lr_arch, lr[lr_arch][rd]))
        print("Learning rate on %s = %s " % (lr_arch, lr[lr_arch][rd]))
    print("-----")
    res_file.write("time(s)=%i\n" % (int(tot_time)))
    print("Elapsed time (s) = %i\n" % (int(tot_time)))
    print(" ")
...
def fed_export_loss_acc_to_txt(out_folder, N_rd, clt,N_clt_str_format, val_lst):
    ...
    nb_val = len(val_lst)
    if clt == -1:
        res = open(out_folder + "/global"+"_res.res").readlines()
    else:
        res = open(out_folder + "/clt"+format(clt,N_clt_str_format)+"_res.res").readlines()

    tr_loss = []
    tr_acc = []
    val_loss = np.ndarray((nb_val, N_rd))
    val_acc = np.ndarray((nb_val, N_rd))
    ...
    # Saving to files
    if clt == -1:
        np.savetxt(out_folder + "/generated_outputs/"+"global_"+"tr_loss.txt", np.asarray(tr_loss), "%0.3f", delimiter=",")
        np.savetxt(out_folder + "/generated_outputs/"+"global_"+"tr_acc.txt", np.asarray(tr_acc), "%0.3f", delimiter=",")
        for i in range(nb_val):
            np.savetxt(out_folder + "/generated_outputs/val_" + "global_"+str(i) + "_loss.txt", val_loss[i], "%0.5f", delimiter=",")
            np.savetxt(out_folder + "/generated_outputs/val_" + "global_"+str(i) + "_acc.txt", val_acc[i], "%0.5f", delimiter=",")
    else:
        np.savetxt(out_folder + "/generated_outputs/"+"clt"+format(clt, N_clt_str_format)+"_"+"tr_loss.txt", np.asarray(tr_loss), "%0.3f", delimiter=",")
        np.savetxt(out_folder + "/generated_outputs/"+"clt"+format(clt, N_clt_str_format)+"_"+"tr_acc.txt", np.asarray(tr_acc), "%0.3f", delimiter=",")

        for i in range(nb_val):
            np.savetxt(out_folder + "/generated_outputs/val_" + "clt"+format(clt, N_clt_str_format)+"_"+str(i) + "_loss.txt", val_loss[i], "%0.5f", delimiter=",")
            np.savetxt(out_folder + "/generated_outputs/val_" + "clt"+format(clt, N_clt_str_format)+"_"+str(i) + "_acc.txt", val_acc[i], "%0.5f", delimiter=",")
...
def fed_create_curves(out_folder, N_rd, clt, N_clt_str_format,val_lst):
    ...
    print("-----")
    print("Generating output files and plots ... ")
    #export_loss_acc_to_txt(out_folder, N_rd, val_lst)
    fed_export_loss_acc_to_txt(out_folder, N_rd,clt,N_clt_str_format, val_lst)

    if not os.path.exists(out_folder + "/generated_outputs"):
        sys.stdacc.write("accOR: No results generated please call export_loss_err_to_txt() before")
        sys.exit(0)
    if clt == -1:
        nb_epoch = len(open(out_folder + "/generated_outputs/"+"global_"+"tr_loss.txt", "r").readlines())
    else:
        nb_epoch = len(open(out_folder + "/generated_outputs/"+"clt"+format(clt, N_clt_str_format)+"_"+"tr_loss.txt", "r").readlines())
    x = np.arange(nb_epoch)
    nb_val = len(val_lst)

    # Loading train Loss and acc
    if clt == -1:
        tr_loss = np.loadtxt(out_folder + "/generated_outputs/"+"global_"+"tr_loss.txt")
        tr_acc = np.loadtxt(out_folder + "/generated_outputs/"+"global_"+"tr_acc.txt")
        # Loading val loss and acc
        val_loss = []
        val_acc = []
        for i in range(nb_val):
            val_loss.append(np.loadtxt(out_folder + "/generated_outputs/val_" +"global_"+ str(i) + "_loss.txt"))
            val_acc.append(np.loadtxt(out_folder + "/generated_outputs/val_" +"global_"+str(i) + "_acc.txt"))
    else:
        tr_loss = np.loadtxt(out_folder + "/generated_outputs/"+"clt"+format(clt, N_clt_str_format)+"_"+"tr_loss.txt")
        tr_acc = np.loadtxt(out_folder + "/generated_outputs/"+"clt"+format(clt, N_clt_str_format)+"_"+"tr_acc.txt")

        # Loading val loss and acc
        val_loss = []
        val_acc = []
        for i in range(nb_val):
            val_loss.append(np.loadtxt(out_folder + "/generated_outputs/val_" +"clt"+format(clt, N_clt_str_format)+"_"+ str(i) + "_loss.txt"))
            val_acc.append(np.loadtxt(out_folder + "/generated_outputs/val_" + "clt"+format(clt, N_clt_str_format)+"_"+str(i) + "_acc.txt"))
    ...
    plt.legend()
    if clt == -1:
        plt.savefig(out_folder + "/generated_outputs/"+"global_"+"loss.png") 
    else:
        plt.savefig(out_folder + "/generated_outputs/"+"clt"+format(clt, N_clt_str_format)+"_"+"loss.png") 
    ...
    plt.legend()
    if clt == -1:
        plt.savefig(out_folder + "/generated_outputs/"+"global_"+"acc.png")
    else:
        plt.savefig(out_folder + "/generated_outputs/"+"clt"+format(clt, N_clt_str_format)+"_"+"acc.png")
    plt.gcf().clear()
...
def expand_str_rd_ep(str_compact, type_inp,N_rd, N_ep, split_elem, mult_elem):
    ...
    if len(str_compact_lst) == 1 and len(elements) == 1:
        lst_out.extend([elements[0] for i in range(N_rd*N_ep - 1)])

    # Final check
    if len(lst_out) != N_rd*N_ep:
        sys.stderr.write(
            'The total number of elements specified in the string "%s" is equal to %i not equal to the total number of epochs %s.\n'
            % (str_compact, len(lst_out), N_rd*N_ep)
        )
        sys.exit(0)

    return lst_out

def expand_str_rd(str_compact, type_inp,N_rd, split_elem, mult_elem):
    ...
    if len(str_compact_lst) == 1 and len(elements) == 1:
        lst_out.extend([elements[0] for i in range(N_rd - 1)])

    # Final check
    if len(lst_out) != N_rd:
        sys.stderr.write(
            'The total number of elements specified in the string "%s" is equal to %i not equal to the total number of epochs %s.\n'
            % (str_compact, len(lst_out), N_rd)
        )
        sys.exit(0)
...
def save_globa_model(nns, multi_gpu, optimizers, info_file, arch_dict):
    for net in nns.keys():
        checkpoint = {}
        if multi_gpu:
...
def global_dict_fea_lab_arch(global_config):
    N_rd = int(global_config["federated"]["round"])
    N_rd_str_format = "0" + str(max(math.ceil(np.log10(N_rd)), 1)) + "d"
    N_ep = int(global_config["federated"]["epoch"])
    N_ep_str_format = "0" + str(max(math.ceil(np.log10(N_ep)), 1)) + "d"
    N_clt = int(global_config["federated"]["client_num"])
    N_clt_str_format = "0" + str(max(math.ceil(np.log10(N_clt)), 1)) + "d"
    tr_data_lst = global_config["data_use"]["train_with"].split(",")
    N_ck = int(global_config[cfg_item2sec(global_config, "data_name", tr_data_lst[0])]["n_chunks"])
    N_ck_str_format = "0" + str(max(math.ceil(np.log10(N_ck)), 1)) + "d"

    
    configfile = global_config["exp"]["out_folder"] + "/exp_files/train_" + tr_data_lst[0]+"_rd"+format(0,N_rd_str_format)+"_ep"+format(0,N_ep_str_format)+"_clt"+format(0, N_clt_str_format)+"_ck"+ format(0,N_ck_str_format) + ".cfg"
    shared_list = []
    read_lab_fea(configfile,False, shared_list, global_config["exp"]["out_folder"])
    fea_dict_used = shared_list[2]
    lab_dict_used = shared_list[3]
    arch_dict_used = shared_list[4]
